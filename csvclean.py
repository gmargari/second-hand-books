...
row_sort = lambda row: (strclean(row[1].lower()), strclean(row[0].lower()), row[4].lower())

reader = csv.reader(open(infile, encoding='utf8'))
header = next(reader)
unique_rows = set(tuple(x) for x in reader)
sorted_rows = [(strclean(title), strclean(author), price, normal_price, url) for title, author, price, normal_price, url in sorted(unique_rows, key=row_sort)]
